...
| CloudWatch Log Group (Flow Logs) | Stores VPC Flow Log entries | No — billed per GB ingested + stored | ~$0.50-0.76/GB ingested + ~$0.03/GB/month storage; negligible at test volume | No — 1-day retention auto-expires old data; delete the log group itself if pausing for cost hygiene, otherwise safe to leave since retention keeps it small |
| IAM Role (vpc-flow-logs-role) | Grants Flow Logs service permission to write to CloudWatch | Yes — always free | $0 | No — free regardless of duration |
| VPC Flow Log | Captures ALL VPC network traffic metadata | No — cost is really the CloudWatch Logs ingestion/storage above, the Flow Log resource itself has no separate charge | $0 direct; drives the Log Group cost above | No — tied to the VPC's lifecycle now that it's in the template; deleted automatically with the stack |